...
public class MessageDispatcher {
    ...
    public MessageDispatcher(Map<String, IMessageHandler> map) {
        loadHandlers(map);
    }

    @Getter
    private final List<IMessageHandler> container = new CopyOnWriteArrayList<>();
    private final List<IMessageHandler> groupContainer = new CopyOnWriteArrayList<>();
    private final List<IMessageHandler> privateContainer = new CopyOnWriteArrayList<>();
    ...
    private boolean toContinue(IMessageHandler handler, Message message){
        ...
        if (message.isGroupMsg()) {
            ...
            if(CollectionUtils.isNotEmpty(accessGroups)
                    ...
                    && !handler.bypassGroupRestrictions()){
                return true;
            }
